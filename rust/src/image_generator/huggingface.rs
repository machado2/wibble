...
impl HuggingFaceImageGenerator {
    pub(crate) fn new() -> Self {
        ...
        let api_url = env::var("HUGGINGFACE_API_URL").unwrap_or_else(|_| {
            "https://router.huggingface.co/hf-inference/models/black-forest-labs/FLUX.1-schnell"
                .to_string()
        }); // Default Flux.1-schnell model
        Self {
        ...
        Box::pin(async move {
            ...
            if let Some(content_type) = content_type {
                if !content_type.starts_with("image/") {
                    ...
                    event!(
                        Level::ERROR,
                        "Hugging Face API response body (not image): {:?}",
                        body
                    );
                    return Err(Error::ImageGeneration(format!(
                ...
                event!(
                    Level::WARN,
                    "Content-Type header missing from Hugging Face API response."
                );
            }
